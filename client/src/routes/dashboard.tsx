...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      { title: "Dashboard | Neptune" },
      {
        name: "description",
        content: "Manage and track all your generated quotations.",
      },
    ],
  }),
  beforeLoad: () => {
...
function DashboardInner() {
  ...
  const fetchQuotations = async () => {
    try {
      ...
      console.warn(
        "[Dashboard] Failed to fetch from API, using local DB:",
        err,
      );
      setQuotations(db.quotations);
    } finally {
  ...
  const updateLocalQuotationStatus = (id: string, newStatus: string) => {
    try {
      ...
      if (raw) {
        ...
        dbVal.quotations = dbVal.quotations.map((item: any) =>
  ...
  const handleCopy = (item: any) => {
    ...
    const total =
      item.totalAmount !== undefined
        ...
        : item.items.reduce(
            (s: number, it: any) =>
              s + it.quantity * (it.price || it.unitPrice),
            0,
          );
  ...
  const getStatusOutlineClass = (s: string) => {
    switch (s) {
      ...
      case "Accepted":
        return "border-emerald-500/80 text-emerald-600 bg-emerald-50/50 hover:bg-emerald-100/60 dark:bg-emerald-950/20";
      case "Rejected":
        return "border-rose-500/80 text-rose-600 bg-rose-50/50 hover:bg-rose-100/60 dark:bg-rose-950/20";
      default:
  ...
  const stats = useMemo(() => {
    ...
    const revenue = quotations.reduce(
      (s, x) =>
        s +
        (x.totalAmount !== undefined
          ? x.totalAmount
          : x.items.reduce(
              (a: number, it: any) =>
                a + it.quantity * (it.price || it.unitPrice),
              0,
            )),
      0,
    );
    const accepted = quotations.filter((x) => x.status === "Accepted").length;
    const counts = { Draft: 0, Sent: 0, Accepted: 0, Rejected: 0 } as Record<
      string,
      number
    >;
    ...
    return {
      ...
      revenue,
      accepted,
      conv: total ? Math.round((accepted / total) * 100) : 0,
      counts,
    };
  }, [quotations]);
    ...
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Track every quote, every status, every rupee.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Link
            to="/inventory"
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-background border border-border text-foreground font-medium shadow-sm hover:bg-muted transition-colors"
          >
            <Package className="w-4 h-4" /> Manage Inventory
          </Link>
          <Link
            to="/quotations/new"
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-primary text-primary-foreground font-medium shadow-elegant hover:opacity-95"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard
          ...
          sub={`${quotations.filter((q) => new Date(q.createdAt).getMonth() === new Date().getMonth()).length} this month`}
        />
        <StatCard
        ...
        <StatCard
          ...
          label="Total Revenue"
          value={formatINR(stats.revenue).replace(/\.00$/, "")}
          sub={`Avg ${stats.total ? formatINR(stats.revenue / stats.total).replace(/\.00$/, "") : "—"}`}
        />
        <StatCard
          icon={<Calendar className="w-5 h-5" />}
          label="Follow-ups"
          value={String(quotations.filter((x) => x.followUpDate).length)}
          sub="due soon"
          tone="violet"
        />
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-3 sm:p-4 shadow-soft flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-3 py-2.5 rounded-xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-ring text-sm"
        ...
        <div className="relative sm:w-44">
          <button
            ...
            className="w-full flex items-center justify-between gap-2 px-4 py-2.5 rounded-xl border border-border bg-background text-sm font-medium hover:bg-muted transition-all"
          >
            <div className="flex items-center gap-2">
              <span
            ...
            <ChevronDown
              className={`w-4 h-4 transition-transform ${statusOpen ? "rotate-180" : ""}`}
            />
          </button>

          {statusOpen && (
            <div className="absolute right-0 mt-2 w-48 rounded-xl border border-border bg-popover shadow-elegant py-1 z-30">
        ...
        <Link
          to="/quotations/new"
          className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-primary text-primary-foreground font-medium shadow-elegant hover:opacity-95 transition-all shrink-0"
        >
          <Plus className="w-4 h-4" /> New Quotation
        </Link>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((q, i) => {
                ...
                const qCustomerName =
                  q.customerSnapshot?.customerName || q.customerName;
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">
                      {q.followUpDate
                        ? new Date(q.followUpDate).toLocaleDateString("en-IN", {
                            day: "2-digit",
                            month: "short",
                          })
                        : "N/A"}
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        ...
                        <Link
                          to={`/quotations/new?copyFrom=${qId}`}
                          ...
                          <button
                            onClick={() =>
                              setActiveStatusMenuId(
                                activeStatusMenuId === qId ? null : qId,
                              )
                            }
              ...
              <div key={qId} className="p-4 space-y-3">
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {/* Delete */}
                    <IconBtn onClick={() => handleDelete(qId)} label="Delete">
                      <Trash2 className="w-4 h-4 text-destructive" />
                    </IconBtn>
                    {/* Status */}
                    <div className="relative">
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
      <div className="flex items-start justify-between">
        <div
          className={`w-10 h-10 rounded-xl grid place-items-center ${toneCls}`}
        >
          {icon}
        </div>
        <div className="text-xs text-muted-foreground text-right">{label}</div>
      </div>
      <div className="mt-4 font-display text-2xl sm:text-3xl font-semibold">
        {value}
      </div>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`text-[10px] uppercase tracking-wider font-medium px-2 py-1 rounded-full ${cls}`}
    >
      {status}
    </span>
  );
...
function ShareDialog({
  ...
  quotationId: string;
  onClose: () => void;
  onSend: (id: string, markSent: boolean) => void;
}) {
      ...
      <div className="w-full max-w-sm rounded-2xl border border-border bg-card shadow-elegant animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <h2 className="font-display font-semibold text-lg flex items-center gap-2">
            <Send className="w-4 h-4 text-primary" /> Share Quotation
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-5 space-y-4">
          <label className="block">
            ...
            <input
              ...
              className="w-full mt-1.5 px-3 py-2.5 rounded-xl border border-border bg-background focus:outline-none focus:ring-2 focus:ring-ring text-sm"
              autoFocus
            />
          </label>
        ...
        <div className="px-5 py-4 border-t border-border flex justify-end gap-2 bg-muted/20 rounded-b-2xl">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-xl border border-border bg-card hover:bg-muted text-sm font-medium transition-colors"
          >
            Cancel
          </button>
          <button
            onClick={() => {
              ...
              onSend(quotationId, markSent);
            }}
            className="px-4 py-2 rounded-xl bg-gradient-primary text-primary-foreground text-sm font-medium shadow-elegant hover:opacity-95 transition-opacity"
          >
            Send Email
          </button>
        </div>
